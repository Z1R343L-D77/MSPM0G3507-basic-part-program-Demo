...
// 定义键值对象数组
comkey_t Key_0,Key_1,Key_2,Key_3,Key_D3_1,Key_D3_2,Key_D3_3,Key_D3_4,Key_D3_5;
//******************************************
...
void key_init(void)
{
	...
	//
	ComKey_Init(&Key_D3_5, 10);//上
	ComKey_Init(&Key_D3_1, 10);//下
	ComKey_Init(&Key_D3_2, 10);//左
	ComKey_Init(&Key_D3_3, 10);//右
	ComKey_Init(&Key_D3_4, 10);//中
}
//******************************************
 void ComKey_GetValue(comkey_t *key) 
 {
     ...
     if (key == &Key_3) {
	  ...
	  //五向按键
	  if (key == &Key_D3_1) {
         key->val = !DL_GPIO_readPins(GPIO_KEY_PORT, GPIO_KEY_D3_1_PIN);
     }
	  if (key == &Key_D3_2) {
         key->val = !DL_GPIO_readPins(GPIO_KEY_PORT, GPIO_KEY_D3_2_PIN);
     }
	  if (key == &Key_D3_3) {
         key->val = !DL_GPIO_readPins(GPIO_KEY_PORT, GPIO_KEY_D3_3_PIN);
     }
	  if (key == &Key_D3_4) {
         key->val = !DL_GPIO_readPins(GPIO_KEY_PORT, GPIO_KEY_D3_4_PIN);
     }
	  if (key == &Key_D3_5) {
         key->val = !DL_GPIO_readPins(GPIO_KEY_PORT, GPIO_KEY_D3_5_PIN);
     }
     //此处添加其他按键，如上
     key->preVal = key->val;
 }
